...
# array of prime #s
# while loop until we find nearest prime #
# jump n times down or up the list
# replace value in array, with new prime #

# switch: if n is positive, switch on once we find a prime higher than arr.max. We will turn it off once we add n elements while switch == true
# opposite for a negative num 
...
def pretentious_primes(arr, num)
    ...
    if num > 0
        ...
        arr.map! do |x|
            new_num = primes.index(find_prime_positive(primes, x))
            primes[new_num + (num - 1)]
        end
    arr
    else 
        answer = []
        arr.each do |y|
            if counter < y && prime?(counter)
                primes << counter
                counter += 1
            end
            if (-1 + num * -1) > primes.length
                answer << nil
            else
                answer << primes[-1 + num]
            end
...
def find_prime_positive(array, target)
    array.each do |x|
        if x > target
            return x
        end
...
# def find_prime_negative(array, target)
