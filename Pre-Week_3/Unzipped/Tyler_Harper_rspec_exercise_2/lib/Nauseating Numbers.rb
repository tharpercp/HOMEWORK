...
def triangular_word?(str)
    alphabet = ("a".."z").to_a
    number = 0
    str.each_char { |char| number += alphabet.index(char) + 1 }
    i = 1
    x = 0
    while x < number
        x = (i * (i + 1)) / 2
...
# find out the number for the word
# test that number by running a triangular number function, stopping when we pass or hit our number
# if we hit, we return true, if we pass, we return false
