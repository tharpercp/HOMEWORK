...
def top_titles
  ...
  top_titles = Movie.select("movies.id, movies.title").where("movies.score >= 9")
...
def star_wars
  ...
  # hint: use 'select' and 'where'
  star_wars = Movie.select("movies.id, movies.title, movies.yr").where("movies.title LIKE 'Star Wars%'")
...
def below_average_years
  ...
  b_a_y = Movie.select("COUNT(movies.*) as bad_movies, yr").where("movies.score < 5").group("movies.yr").order("COUNT(movies.*) DESC")
...
def alphabetized_actors
  ...
  alpha_actors = Actor.select("actors.*").order("actors.name").limit("10")
